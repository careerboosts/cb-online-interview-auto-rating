...
def get_employee_groups(employee_id, projectId):
  project = Projects_collection.find_one({"_id": projectId})
  if not project:
      # Simulating an error
  ...
  group_info = []
  for project_group in projectGroups:
      try:
          group_employees = project_group['groupusers']

          for emp in group_employees:
              try:
                  if emp == (employee_id):
                      group_info.append(project_group)
              except Exception as e:
                  logging.error(str(e))
      except Exception as e:
          logging.error(str(e))
...
def createManager(
    ...
    user_doc = Users_collection.find_one({"userEmail": email})
    if user_doc: return print("User with this email address already exists!")

    try:
      user_doc = Users_collection.insert_one({
      ...
      print("new user created in the db")
    except RuntimeError as error:
      print("Error while creating new user on the db: " + error)

    if user_doc:
        try:
          manager_doc = Managers_collection.insert_one({
            "managerUserId": user_doc.inserted_id,
            "managerProjectIds": [ObjectId(projectId)],
            "seeScore": False,
            "managerEmployees": employees,
            "testAdmin": False
          })
          print("new manager created in the db")
        except RuntimeError as error:
          print("Error while creating new manager on the db: " + error)
    ...
    if manager_doc:
      payload = {
        ...
        "enabled": "true",
        "groups": ["manager", "employee"] if isEmployee else ["manager"]
      }
      try:
        createUser(payload)
        print("new user created in keycloak")
      except RuntimeError as error:
        print("Error while creating new user on keycloak: " + error)

      if isEmployee and groupId:
          try:
            Employees_collection.insert_one({
              "employeeUserId": user_doc.inserted_id,
              "employeeProjectId": ObjectId(projectId),
              "employeeCorporate_id": corporateId,
              "employeeRoles": roles,
              "employeeSelectedLanguage": selectedLanguage
            })
            Projects_collection.update_one(
              {"_id": ObjectId(projectId), "projectGroups.group_id": groupId},
              {"$push": {"projectGroups.$.groupusers": user_doc.inserted_id}}
            )
            Participations_collection.insert_one({
               "participationUserId": user_doc.inserted_id,
               "participationProjectId": ObjectId(projectId),
               "participationStatus": "not started",
               "participationProgress": []
            })
            print("new employee created in the db")
          except RuntimeError as error:
            print("Error while creating new employee on the db: " + error)

      project_doc = Projects_collection.find_one({"_id": ObjectId(projectId)})

      if project_doc:
          if "scoreReleased" in project_doc.keys():
            Managers_collection.update_one(
              {"_id": manager_doc.inserted_id},
              {"$set": {"seeScore": project_doc["scoreReleased"]}}
            )
          else:
            Projects_collection.update_one(
              {"_id": ObjectId(projectId)},
              {"$set": {"scoreReleased": False}}
            )

          if "projectManagers" in project_doc.keys():
            Projects_collection.update_one(
              {"_id": ObjectId(projectId)},
              {"$push": {"projectManagers": user_doc.inserted_id}}
            )
          else:
            Projects_collection.update_one(
              {"_id": ObjectId(projectId)},
              {"$addToSet": {"projectManagers": user_doc.inserted_id}},
              upsert=True
            )


# createManager(
#   email="manger11-test@manager.com", 
#   firstName="manager", 
#   lastName="test",
#   middleName="",
#   phone="",
#   password="Abcd1234*$",
#   isEmployee=True,
#   projectId="64c11da4225b4cf0c1fa4ad8",
#   employees=[ObjectId("64c11e14225b4cf0c1fa4adb")],
#   groupId="group_ 0"
# )

def addCompetencyLanguage(lang):
  Competencies_collection.update_many({"competencyLanguage": {"$exists": False}}, {"$set": {"competencyLanguage": lang}})

def addRoleLanguage(lang):
  Roles_collection.update_many({"roleLanguage": {"$exists": False}}, {"$set": {"roleLanguage": lang}})

def addCourseCompetencies():
  Courses_collection.update_many({"courseCompetencies": {"$exists": False}}, {"$set": {"courseCompetencies": []}})

def addcertificationCompetencies():
  Certifications_collection.update_many({"certificationCompetencies": {"$exists": False}}, {"$set": {"certificationCompetencies": []}})

# addCompetencyLanguage("English")
# addRoleLanguage("English")
# addCourseCompetencies()
# certificationCompetencies()